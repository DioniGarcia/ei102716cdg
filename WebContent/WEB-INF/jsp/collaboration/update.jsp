<%@page contentType="text/html; charset=iso-8859-1"%>
<%@taglib uri="http://www.springframework.org/tags/form" prefix="form"%> 
<%@taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c" %>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8" />
<title>Skill Sharing - Modificar Colaboración</title>
</head>
<body>
    <h2>Editar Colaboración</h2>
    <form:form method="post" modelAttribute="collaboration">
        <table>
            <tr>
                <td><form:label path="startDate">Fecha de inicio</form:label></td>
                <td><form:input type="date" path="startDate" /></td>
            </tr>
            
            <tr>
            <!-- Linea separadora --><td><hr/></td> <td><hr/></td> <td><hr/></td>
            </tr>
            
             <tr>
                <td><form:label path="startDate">Fecha final</form:label></td>
                <td><form:input type="date" path="endDate" /></td>
            </tr>
            
            <tr>
            <!-- Linea separadora --><td><hr/></td> <td><hr/></td> <td><hr/></td>
            </tr>
            
            <tr>
                <td><form:label path="comments">Comentario</form:label></td>
                <td colspan="3"><form:input  path="comments" /></td>
            </tr>
            
            <tr>
            <!-- Linea separadora --><td><hr/></td> <td><hr/></td> <td><hr/></td>
            </tr>
            
            <tr>
                <td><form:label path="rating">Puntuación</form:label></td>
                <td>
                	<form:select path="rating">
						<form:option value="1" />
						<form:option value="2" />
						<form:option value="3" />
						<form:option value="4" />
						<form:option value="5" />
					</form:select >
				</td>
            </tr>


            <tr>
            	<td><hr/></td> <!-- Linea separadora -->
            	<td><hr/></td>
            </tr>
            
            <tr>
                <td><form:label path="offerId">Skill</form:label></td>
                <td>
                	<form:select path="offerId">
                		<c:forEach items="${offers}" var="offer">
	 						<form:option value="${offer.id}">${offer.description}</form:option>
	 					</c:forEach>
					</form:select >
				</td>
            </tr>
            
            <tr>
            	<td><hr/></td> <!-- Linea separadora -->
            	<td><hr/></td>
            </tr>
            
            <tr>
                <td><form:label path="requestId">Skill</form:label></td>
                <td>
                	<form:select path="requestId">
                		<c:forEach items="${requests}" var="request">
	 						<form:option value="${request.id}">${request.description}</form:option>
	 					</c:forEach>
					</form:select >
				</td>
            </tr>
            
            <tr></tr>
            
            <tr>
                <td colspan="2"><input type="submit" value="Añadir" />
                </td>
            </tr>
        </table>
    </form:form>
</body>
</html>
